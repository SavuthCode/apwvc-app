...
class RoleRepository
{
    ...
    {
        $lims_role_data = Roles::where('id', $data['role_id'])->first();
        $lims_role_data->update($data);
        return $lims_role_data->fresh();
    }

    public function setPermissionRepository($request)
    {
        ...
        if($request->has('attribute')) {
            $permission = Permission::firstOrCreate(['name' => 'attribute']);
            if(!$role->hasPermissionTo('attribute')) {
                $role->givePermissionTo($permission);
            }
        }
        else
            $role->revokePermissionTo('attribute');
        return $role;
    }
